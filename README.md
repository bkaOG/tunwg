...
or
```bash
tunwg --tunnel=http://localhost:8080
```
You can run tunwg in docker too:
```bash
docker run -it --rm --network=host -v tunwg_keys:/data ghcr.io/ntnj/tunwg tunwg --tunnel=http://localhost:8080
```
...
Tunwg provides end to end encryption by forwarding TCP streams over wireguard to the tunwg instance running on your local machine. The local instance forwards the traffic to your local server. This means that your traffic is completely private to you.
...
Automatic SSL certificates can be handled by a reverse proxy (like Caddy or nginx) in front of the tunwg server.
...
```bash
tunwg --tunnel=http://10.0.0.2:8000,http://10.0.0.10:9000
```
...
```docker-compose.yml
  tunwg:
    ...
    command: tunwg --tunnel=http://whoami
...
```bash
tunwg --tunnel=... --limit=$(htpasswd -nbB <user> <password>)
```
...
Even though the subdomain generated by tunwg seems random, they're not private. Since SSL certificates may be issued for them, the encoded subdomain could be added to certificate transparency logs if using a reverse proxy with automatic certificates. Many crawlers and attackers monitor those transparency logs, so you'll get some automated traffic to your servers after you first issue the certificate. Do not forward any local servers which may be vulnerable or expose private data without auth, and if you need to do that, use the inbuilt tunwg basic auth.
...
```bash
go install github.com/ntnj/tunwg/tunwg@latest
TUNWG_RUN_SERVER=true TUNWG_API=example.com TUNWG_IP=<ip-of-server> TUNWG_PORT=<wireguard-port> TUNWG_HTTP_PORT=<http-port> tunwg
```
...
tunwgs:
  image: ghcr.io/ntnj/tunwg
  network_mode: host  # or ports, <http-port>
  environment:
    ...
    TUNWG_HTTP_PORT: 8080  # http port for the server (default: 80)
```
...
The server listens on the configured HTTP port (default 80) for HTTP traffic. It also listens on UDP port `TUNWG_PORT` for wireguard UDP traffic. The public instance listens on UDP 443, since it's less likely to be blocked by firewalls.
...
You should run the tunwg server behind a reverse proxy like Caddy or nginx that handles SSL/TLS termination and forwards HTTP traffic to the tunwg server.
...
The generated domain name is an encoding of the internal wireguard IP address and the port. When tunwg server receives a request, it parses the HTTP Host header to get the domain and decodes it to an IP:port pair, which it then forwards the connection to over the internal wireguard network.

### Develop locally

Run server: `TUNWG_TEST_LOCALHOST=true TUNWG_RUN_SERVER=true TUNWG_KEY=tunwgs TUNWG_PORT=443 TUNWG_IP=127.0.0.1 TUNWG_HTTP_PORT=8080 go run ./tunwg`

Run client: `TUNWG_TEST_LOCALHOST=true go run ./tunwg --tunnel=http://localhost:8000`

Test: `curl -Li --connect-to ::127.0.0.1:8080 http://abcd.l.tunwg.com`
